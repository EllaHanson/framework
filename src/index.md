...
const locations_input = await FileAttachment("data/locations.json").json();
const state_locations = new Map(locations_input.map(i => [i.state, [i.lon, i.lat]]));
...
// load up stuff from CleanSnapData.json
...
function bubbleChart(data, {
  ...
  // get the geo location for the states
  const projection = d3.geoAlbersUsa()
    .fitExtent([[0, 0], [width, height]], {
      type: "FeatureCollection",
      features: Array.from(state_locations, ([state, [lon, lat]]) => ({
        type: "Feature",
        properties: { state },
  ...
  const nodes = (data
    .map(i => {
        const loc = state_locations.get(i.state);
        if (!loc) return null;
        ...
        return {
          ...
          tx: point[0],
          ty: point[1],
          r: rScale(rAccessor(i))
        };
      })
      .filter(Boolean));
  
  const simNodes = nodes.map(n => ({ ...n, x: n.tx, y: n.ty }));
  // force to where state should try to be
  const sim = d3.forceSimulation(simNodes)
    .force("x", d3.forceX(d => d.tx).strength(0.25))
    .force("y", d3.forceY(d => d.ty).strength(0.25))
    // dont overlap with other states
    .force("collide", d3.forceCollide(d => d.r + 1.5))
    .stop();

  // let physics sim do its thing for a bit
...
const months = Array.from(
  new Set(input.map(d => format(new Date(d.date))))
).sort();
